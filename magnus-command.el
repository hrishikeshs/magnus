...
;; interface for interacting with all Claude Code instances.  Events
;; (auto-approvals, vterm prompts, agent replies, messages) appear in a
;; single chronological stream with an inline input area at the bottom.
;;
;; Messages are sent via @agent-name or C-c C-t to pick a target.
;; Permission prompts use the hook → SKIP → vterm flow (see
;; magnus-permission.el); this buffer is a pure log/awareness layer.
...
(defun magnus-command--insert-input-prompt ()
  "Insert the input prompt text based on target."
  (let ((prompt-text
         (cond
...
Otherwise, send as a free-form message via @mention or target."
  (interactive)
...
(defun magnus-command--render-prompt-header (event)
  "Insert the header for a legacy prompt EVENT (always rendered as handled)."
  (let* ((ts (magnus-command--format-timestamp (plist-get event :timestamp)))
         (name (plist-get event :instance-name))
         (label (format "(approved: %s)" (or (plist-get event :response) "?"))))
...
(defun magnus-command--poll-jsonl ()
  "Poll JSONL files for agent replies."
  (when-let ((buf (get-buffer magnus-command-buffer-name)))
    (with-current-buffer buf
      (dolist (instance (magnus-instances-list))
